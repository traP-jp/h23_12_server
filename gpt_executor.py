...
async def query_gpt(client, prompt):
    """
    ...
    try:
        ...
        if not response:
            raise HTTPException(status_code=500, detail="No response from GPT API")
        ...
        if not match:
            raise HTTPException(status_code=500, detail="No detailed description found in GPT response")
...
async def query_dall_e(client, prompt):
    """
    ...
    try:
        ...
        if not response:
            raise HTTPException(status_code=500, detail="No response from DALL-E API")

        # 生成された画像のURLを取得
        image_url = response.data[0].url
        # ToDo: I have to check that it is enough the unexpected error.
        return image_url
    except Exception as e:
